...
REDIPS.drag = (function () {
	...
	var	init,						// initialization
		...
		mark = {action: 'deny',
				cname: 'mark',
				exception: []},
		border = 'none',				// (string) border style for enabled elements
	...
	init = function (dc) {
		...
		var len = $(div_drag).find(".drag").length;
		var elemnt ;
		var pos;
		for(var i=0;i<len;i++){
//				alert($(div_drag).find(".drag").get(i).obj);
				elemnt = ($(div_drag).find(".drag").get(i));
				if($(elemnt).parent().parent().attr("id")!="firstRow"){
				pos = get_position(elemnt);
//				alert($(elemnt).parent().parent());
...
//				else{
//					pos = get_position(elemnt);
//					$(elemnt).parent().parent().find("#page").val(pos[0]);
//				}
	...
	handler_onmousedown = function (e) {
		var evt = e || window.event,	// define event (cross browser)
			...
			Y;						// X and Y position of mouse pointer
		...
		var el = REDIPS.drag.obj;
		// find parent table
		el = REDIPS.drag.find_parent('TABLE', el);
		if($(el).attr("id")!="masterTable"){
			if($(el).attr("class")=="deletedRows")
				isRow = true;
			else
				isRow = false;
			$("#masterTable").attr("class","nolayout");
		}
		else{
