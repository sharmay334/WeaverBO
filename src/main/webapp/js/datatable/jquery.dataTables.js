...
(/** @lends <global> */function( $ ) {
	...
	function _fnAddOptionsHtml ( oSettings )
	{
		...
		for ( var i=0 ; i<aDom.length ; i++ )
		{
			...
			{
				/* Length */
				featureNode = $('<div/>',{id:oSettings.sTableId+'_length_filter', 'class':'dataTables_length_filter'}).append(_fnFeatureHtmlLength( oSettings ));
			}
			...
			if ( featureNode )
			{
				...
				if(cOption == 'f' && insert.find('#'+oSettings.sTableId+'_length_filter')[0]){
					insert.find('#'+oSettings.sTableId+'_length_filter').append( featureNode );
				}else{
	...
	function _fnBuildAjax( oSettings, data, fn )
	{
		...
		var baseAjax = {
			...
			"error": function (xhr, error, thrown) {
				...
				if((xhr.responseText).contains("sessionTimeOut.action")){
					window.location="com/stpl/pms/common/execption/nodecorate/sessionTimeOut.jsp";
					return false;
				}
	...
	DataTable.defaults = {
		 ...
		 *    } );
		 */
		"aLengthMenu": [ 10, 25, 50, 100, 10000 ],
...
// ========== (AMIT GARG) ================
// for column sorting which has numeric value as well as String('--') values. 
//By default datatable treat this kind of column as a string. 
var regs = new RegExp(/^(\d{2})\/(\d{2})\/(\d{4})([\w ]?)((\d{2}):(\d{2}):(\d{2}))?$/);

$.fn.dataTable.ext.type.detect.unshift(function ( d ) {
	if(d!=null && d!=""){
		d = d.toString();
		if(regs.test(d) || !isNaN(parseFloat(d)) || d=="--") {
...
$.fn.dataTable.ext.type.order['customSorting-pre'] = function ( a ) {
	if(a!=null){
		a = a.toString();
		if(a=="--"){
			return -1;
		    ...
		    if(dateTime.length>1){
		    	...
		    	dateSum = (date[2] + date[1] + date[0] + time[0] + time[1] + time[2]) * 1;
			} else {
				dateSum = (date[2] + date[1] + date[0]);
			}
			return dateSum;
		} else {
			return parseFloat((isNaN(parseFloat(a))) ? -1 : a);
		}
	}
};