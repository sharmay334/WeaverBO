function chkDefault(objArr) {
		...
		/*if(count>0){
			defChk.attr('checked',true);
			defChk.attr('readonly',true);
		}else{
			//defChk.attr('checked',false);
			defChk.attr('readonly',false);
		}
		/*if(obj.val().indexOf('*')==-1){
			...
			/*obj.closest('table').parents('td').find('.chkOne').each(function(){
				...
				if($(this).val().indexOf('*')==-1 && this.checked){
					count++;
				}
			});*/
		//alert(count);
	...
	/*function getUserPriveledges(userName) {
		if (userName == -1) {
			...
			var actionName = 'bo_um_editBoSubUserPriv_FetchPriv.action';
			var actionParam='userName='+userName;
			_ajaxCallDiv(actionName,actionParam,'result');
		}
		return false;
	}*/
	...
	function checkUploadFrm(){
		...
		else if(frm.bannerName.value.length < 3 || frm.bannerName.value.length > 30 ){
		...
		if(frm.bannerURL.value==""){
			...
			frm.bannerURL.focus();
			return false;
		}else if(frm.bannerURL.value.indexOf("http:") != -1 || frm.bannerURL.value.indexOf("ftp:") != -1 || frm.bannerURL.value.indexOf("https:") != -1){
		...
		if(txtElm.className == "show"||txtElm.className==""){
		 ...
		 if(flashElm1.className == ""&&flashElm2.className == ""){
			...
			if(frmBannerDim==null || frmBannerDim==""){
				...
				if(frmBannerDim.indexOf("X") == -1 || frmBannerDim.indexOf("X") == 0 || frmBannerDim.indexOf("X") == frmBannerDim.length-1){
