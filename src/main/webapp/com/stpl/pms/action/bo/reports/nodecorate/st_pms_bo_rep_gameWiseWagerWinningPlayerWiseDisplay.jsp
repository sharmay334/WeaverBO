...
$(document).ready(function(){
		$('#waitDiv').hide();
  		...
  		fromAndToDate();
  		$("#reportTablePlayerWiser").DataTable({
			...
			"oLanguage" : {
			    ...
			    $(document).on("click", "[data-dt-idx='5']:not(.next,.previous)", function(){
 					if($("#rowCount").text()==101 && $("#search").val()==""){
	 					$("#reportTablePlayerWiser_next").removeClass("disabled");
		 				$("[data-dt-idx='5']").addClass('nextCall');
		 			}
 				});
 				$(document).on("click", "[data-dt-idx='1']:not(.next,.previous)", function(){
 					if($("[data-dt-idx='1']").text()>1)
 						$("#reportTablePlayerWiser_previous").removeClass("disabled");
	 				$("[data-dt-idx='1']").addClass('prevCall');
 				});
 				$("#reportTablePlayerWiser_next").live('click', function(e) {
								if($("#rowCount").text()==101 && $("#search").val()==""){
									$("#reportTablePlayerWiser_next").removeClass("disabled");
								}
								if($("#reportTablePlayerWiser_next").hasClass("disabled"))
									return false;
								...
								if($("[data-dt-idx='5']").hasClass("nextCall")  && !($("#reportTablePlayerWiser_next").hasClass("disabled"))) {
									var start = parseInt($(
											"#startIndex")
											.val()) + 100;
											flagPage = 0;
									...
									$('#startIndex').val(start);
									 x = document.getElementById("reportTablePlayerWiser_paginate").lastChild.innerHTML;
									document.getElementById("reportTablePlayerWiser_paginate").lastChild.innerHTML=document.getElementById("waitDivNext").innerHTML;
									$('#reportTablePlayerWiser_paginate > a:last-child').addClass('waitBtn');
							...
							$('#reportTablePlayerWiser_previous').live('click', function(e) {
									if($("[data-dt-idx='1']").text()>1){
 										$("#reportTablePlayerWiser_previous").removeClass("disabled");
 									}
									...
									if($("[data-dt-idx='1']").hasClass("prevCall") && !($("#reportTablePlayerWiser_previous").hasClass("disabled"))) {	
									var start = parseInt($("#startIndex").val()) - 100;
											var nextPageStart = parseInt($('.paginate_button.current').text())-5;
											$("#nextPageStart").val(nextPageStart);
									$('#startIndex').val(start);
									 y = document.getElementById("reportTablePlayerWiser_paginate").firstChild.innerHTML;
									document.getElementById("reportTablePlayerWiser_paginate").firstChild.innerHTML=document.getElementById("waitDivPrev").innerHTML;
									$('#reportTablePlayerWiser_paginate > a:first-child').addClass('waitBtn');
									$(".paginate_button").css("pointer-events","none");
									_ajaxCallJsonSynch(
											"com/stpl/pms/action/bo/reports/st_pms_bo_rep_searchMISReportOnClick.action?reportData="
													+$(
		...
		var resultList = response;
			...
			if ($("#rowCount").text() < 101){
				$("#reportTablePlayerWiser_next").addClass("disabled");}
			//	alert(response.size());
				var nextPage = $("#nextPageStart").val();
				
			jQuery('#reportTablePlayerWiser').dataTable().fnDestroy();
			$("#" + 'reportTablePlayerWiser')
					.DataTable(
							{
								"sAjaxSource" : 'com/stpl/pms/action/bo/reports/dataTablesActionMis.action',
								"sAjaxDataProp" : "resultList",
								...
								"bSort" : false,
								 "fnInitComplete": function( oSettings) {
 									$("#reportTablePlayerWiser_previous").removeClass("disabled");
 									$("[data-dt-idx='1']").addClass('prevCall');
 									document.getElementById("reportTablePlayerWiser_paginate").lastChild.innerHTML = x;
									$($(".dataTables_paginate>span>.paginate_button")).each(function(){
									$(this).text(nextPage);
									nextPage++;
								});
					...
					.DataTable(
							{
								...
								"bSort" : false,
								 "fnInitComplete": function( oSettings) {
									 if($('#startIndex').val()==0){
										$("#reportTablePlayerWiser_previous").addClass("disabled");}
									 else
 										$("#reportTablePlayerWiser_previous").removeClass("disabled");
 									$("[data-dt-idx='1']").addClass('prevCall');
 									document.getElementById("reportTablePlayerWiser_paginate").firstChild.innerHTML = y;
									$($(".dataTables_paginate>span>.paginate_button")).each(function(){
									$(this).text(nextPage);
									nextPage++;
									$('#reportTablePlayerWiser').dataTable().fnPageChange( 'last' );
									$("#reportTablePlayerWiser_next").removeClass("disabled");
									$("[data-dt-idx='5']").addClass('nextCall');
								});
								},
		...
		<s:if test="%{resultList!=null && resultList.size()>0}">
			...
			<div class="innerBox">
				<table cellspacing="0" cellpadding="4" border="0" align="center"
					class="payTransaction" id="reportTablePlayerWiser">
					<thead>
