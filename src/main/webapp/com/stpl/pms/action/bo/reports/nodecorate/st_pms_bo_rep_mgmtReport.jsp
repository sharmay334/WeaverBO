...
$(document).ready(function(){
			...
			"oLanguage" : {
			    ...
			    $(document).on("click", "[data-dt-idx='5']:not(.next,.previous)", function(){
 					if($("#rowCount").text()==101 && $("#search").val()==""){
	 					$("#mgmtReport_next").removeClass("disabled");
		 				$("[data-dt-idx='5']").addClass('nextCall');
		 			}
 				});
 				$(document).on("click", "[data-dt-idx='1']:not(.next,.previous)", function(){
 					if($("[data-dt-idx='1']").text()>1)
 						$("#mgmtReport_previous").removeClass("disabled");
	 				$("[data-dt-idx='1']").addClass('prevCall');
 				});
 				$("#mgmtReport_next").live('click', function(e) {
								if($("#rowCount").text()==101 && $("#search").val()==""){
									$("#mgmtReport_next").removeClass("disabled");
								}
								if($("#mgmtReport_next").hasClass("disabled"))
									return false;
								...
								if($("[data-dt-idx='5']").hasClass("nextCall")  && !($("#mgmtReport_next").hasClass("disabled"))) {
									var start = parseInt($(
											"#startIndex")
											.val()) + 100;
											flagPage = 0;
									...
									$('#startIndex').val(start);
									 x = document.getElementById("mgmtReport_paginate").lastChild.innerHTML;
									document.getElementById("mgmtReport_paginate").lastChild.innerHTML=document.getElementById("waitDivNext").innerHTML;
									$('#mgmtReport_paginate > a:last-child').addClass('waitBtn');
							...
							$('#mgmtReport_previous').live('click', function(e) {
									if($("[data-dt-idx='1']").text()>1){
 										$("#mgmtReport_previous").removeClass("disabled");
 									}
									...
									if($("[data-dt-idx='1']").hasClass("prevCall") && !($("#mgmtReport_previous").hasClass("disabled"))) {	
									var start = parseInt($("#startIndex").val()) - 100;
											var nextPageStart = parseInt($('.paginate_button.current').text())-5;
											$("#nextPageStart").val(nextPageStart);
									$('#startIndex').val(start);
									 y = document.getElementById("mgmtReport_paginate").firstChild.innerHTML;
									document.getElementById("mgmtReport_paginate").firstChild.innerHTML=document.getElementById("waitDivPrev").innerHTML;
									$('#mgmtReport_paginate > a:first-child').addClass('waitBtn');
									$(".paginate_button").css("pointer-events","none");
									_ajaxCallJsonSynch(
											"com/stpl/pms/action/bo/reports/st_pms_bo_rep_searchMISReportOnClick.action?reportData="
													+$(
		...
		var resultList = response;
			...
			if ($("#rowCount").text() < 101){
				$("#mgmtReport_next").addClass("disabled");}
			//	alert(response.size());
				var nextPage = $("#nextPageStart").val();
				
			jQuery('#mgmtReport').dataTable().fnDestroy();
			$("#" + 'mgmtReport')
					.DataTable(
							{
								"sAjaxSource" : 'com/stpl/pms/action/bo/reports/dataTablesActionMis.action',
								...
								"bSort" : false,
								 "fnInitComplete": function( oSettings) {
 									$("#mgmtReport_previous").removeClass("disabled");
 									$("[data-dt-idx='1']").addClass('prevCall');
 									document.getElementById("mgmtReport_paginate").lastChild.innerHTML = x;
									$($(".dataTables_paginate>span>.paginate_button")).each(function(){
									$(this).text(nextPage);
									nextPage++;
								});
					...
					.DataTable(
							{
								...
								"bSort" : false,
								 "fnInitComplete": function( oSettings) {
									 if($('#startIndex').val()==0){
										$("#mgmtReport_previous").addClass("disabled");}
									 else
 										$("#mgmtReport_previous").removeClass("disabled");
 									$("[data-dt-idx='1']").addClass('prevCall');
 									document.getElementById("mgmtReport_paginate").firstChild.innerHTML = y;
									$($(".dataTables_paginate>span>.paginate_button")).each(function(){
									$(this).text(nextPage);
									nextPage++;
									$('#mgmtReport').dataTable().fnPageChange( 'last' );
									$("#mgmtReport_next").removeClass("disabled");
									$("[data-dt-idx='5']").addClass('nextCall');
								});
								},
		...
		<s:if test="%{resultList!=null && resultList.size()>0}">
			<div class="greyStrip">
				...
				<div class="dataTbl_top">
					<p>
						<a
							href="com/stpl/pms/action/bo/reports/st_pms_bo_rep_playerTxnReportsExportExcel.action?reportData=<s:property value="%{reportData}"/>&reportTypeName=MgmtReport">Download
							Excel</a>
					</p>
				</div>
			<div class="innerBox">
				<table cellspacing="0" cellpadding="4" border="0" align="center"
					class="payTransaction" id="mgmtReport">
					<thead>
						...
						<tr class="headerRow">
							...
							<th valign="middle" style="text-align: center;" nowrap="nowrap">
								...
								Dep-W/D</th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
								No. of Regular Depositors Count</th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
								No. of New  Depositors (Without Ola)</th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
							No. of New OLA Depositors</th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
								Same day depositors from Same day RG ( W/O OLA)</th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
								No. of New  Depositors-Mobile</th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
								Daily Login Count</th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
								Bonus Distributed Amount</th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
								Bonus Release </th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
								Number of Depositors who played cash game Without OLA</th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
								Winning </th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
							OLA Wager</th>
						...
						OLA Winning</th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
							Count of Depositing Players who Played cash Game </th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
							Count of OLA players who played cash games</th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
							No of Free Players</th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
							No. of Total Players played</th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
							Direct Players Wager</th>
								<th valign="middle" style="text-align: center;" nowrap="nowrap">
							Direct Players winning</th>
							<th valign="middle" style="text-align: center;" nowrap="nowrap">
						Same month depositors from Same month RG (W/O OLA)</th>
							<th valign="middle" style="text-align: center;" nowrap="nowrap">
							Net Rake</th>
						</tr>
						</s:if>
