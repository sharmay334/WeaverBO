...
function getTeamTarget(rsmName){
	
var myurl = "<%=basePath%>";
	
	var frm = $('#searchUserFrm');
	myurl += "/com/stpl/pms/action/bo/um/getRSMTeamTargetAmount.action?rsmName="+rsmName+"&subTeamName="+document.getElementById('subTeamName').value;
		...
		$.ajax({
			...
			success : function(result) {
				if(result=="success"){
					swal({
						  title: "Good job!",
						  text: "TSM assigned successfully!",
						  icon: "success",
						  button: "Close",
						});
					setTimeout(function(){
					...
					swal({
						  ...
						  text: "Duplicate record or target amount is greater than Pending Target Amount.",
						  icon: "warning",
						  button: "Close",
						});
				}
...
<body>
	<div class="FormSection">
		...
		<s:form id="searchUserFrm"
			action="/com/stpl/pms/action/bo/um/bo_um_assignTSMSubTeam_submit.action"
			...
			<div class="innerBox">
				...
				<div class="FormMainBox">
					<div class="labelDiv">
						<label>Select Under RSM</label><em class="Req">*</em>
					</div>
					<div class="InputDiv">
						<s:select name="rsmName" headerKey="-1" id="rsmName"
								headerValue="--Please Select--" onChange="getTeamTarget(this.value)" list="empNameList"
				...
				<div class="FormMainBox">
					<div class="labelDiv">
						<label>Select TSM</label><em class="Req">*</em>
					</div>
					<div class="InputDiv">
						<s:select name="tsmName" headerKey="-1" id="tsmName"
								headerValue="--Please Select--" list="empNameList"
								cssClass="select1" theme="myTheme" errorMassage="Select TSM First"/>
						<div id="tsmName_error" class="fieldError">
							<s:fielderror>
								<s:param>tsmName</s:param>
							</s:fielderror>
						</div>
					</div>
				</div>
				...
				<div class="FormMainBox">
					<div class="labelDiv">
						<label>TSM Target Amount</label><em class="Req">*</em>
					</div>
					<div class="InputDiv">
						<ss:textfield maxlength="30" id="tsmTarget" applyscript="true"
							theme="myTheme" name="tsmTarget" pattern="^[0-9.]*$"
							errorMassage="Only Numeric Character not allowed"
							theme="myTheme"></ss:textfield>
						<div id="tsmTarget_error" class="fieldError">
							<s:fielderror>
								<s:param>tsmTarget</s:param>
							</s:fielderror>
						</div>
					</div>
				</div>
