...
<title>Upload Player Bank Document</title>
	...
	$(document).ready(function(){
		...
		$("#playerSearchFrm").submit(function(){
			if(submitFlag)
			{
				...
				displayTable($("#domainId").val(), $("#aliasId").val(),$("#userName").val(),$("#playerId").val(),$("#status").val(),startDateValue,endDateValue);				
			}
	...
	function displayTable(domainId,aliasId,userName,playerId,status,uploadStartDate,uploadEndDate){
		$("#expandIt").html('');
		var flag = 0;
		$("#uploadDoc").DataTable({
			"bDestroy": true,
			"deferRender": true,
			"sAjaxSource" : 'com/stpl/pms/action/bo/pm/bo_pm_search_red_plr.action?domainId='+domainId+'&aliasId='+aliasId+'&userName='+userName+'&playerId='+playerId+'&status='+status+'&uploadStartDate='+uploadStartDate+'&uploadEndDate='+uploadEndDate,
			...
			"aoColumnDefs": [{
				...
				"mRender": function (data,type, full) {
					return '<a href="com/stpl/pms/action/bo/pm/bo_pm_plr_bank_document_detail.action?userName='+full[1]+'&playerId='+full[2]+'&redeemAccId='+full[3]+'&bankIdVerification='+full[4]+'&identityIdVerification='+full[5]+'" target="expandIt" callback="editUploadPlrDocs()" >Edit</a>';
				}
			}],
...
<body>
	<s:form id="playerSearchFrm" theme="simple">
		<div class="FormSection">
			...
			<div class="greyStrip">
				<h4>Upload Player Bank Document</h4>
			</div>
			<div class="innerBox">
				...
				<div class="FormMainBox">
					...
					<div class="InputDivHalf">
						<ss:textfield id="playerId" maxlength="16" pattern="^[0-9]*$" errorMassage="Enter only numeric character" cssClass="text-field-small" theme="myTheme" ></ss:textfield>
					</div>
				</div>
				<div class="clearFRM"></div>
					<div class="FormMainBox">
