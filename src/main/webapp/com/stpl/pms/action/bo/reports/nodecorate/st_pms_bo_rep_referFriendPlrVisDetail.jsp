	...
	$('#waitDiv').hide();
				...
				"bVisible": true, 
 				...
 				$(document).on("click", "[data-dt-idx='5']:not(.next,.previous)", function(){
 					if($("#rowCount").text()==101 && $("#search").val()==""){
	 					$("#reportTable_vis_next").removeClass("disabled");
		 				$("[data-dt-idx='5']").addClass('nextCall');
		 			}
 				});
 				$(document).on("click", "[data-dt-idx='1']:not(.next,.previous)", function(){
 					if($("[data-dt-idx='1']").text()>1)
 						$("#reportTable_vis_previous").removeClass("disabled");
	 				$("[data-dt-idx='1']").addClass('prevCall');
 				});
 				$("#reportTable_vis_next").live('click', function(e) {
								if($("#rowCount").text()==101 && $("#search").val()==""){
									$("#reportTable_vis_next").removeClass("disabled");
								}
								if($("#reportTable_vis_next").hasClass("disabled"))
									return false;
								...
								if($("[data-dt-idx='5']").hasClass("nextCall")  && !($("#reportTable_vis_next").hasClass("disabled"))) {
									var start = parseInt($(
											"#startIndex")
											.text()) + 100;
											flagPage = 0;
									...
									$('#startIndex').text(start);
									 x = document.getElementById("reportTable_vis_paginate").lastChild.innerHTML;
									document.getElementById("reportTable_vis_paginate").lastChild.innerHTML=document.getElementById("waitDivNext").innerHTML;
									$('#reportTable_vis_paginate > a:last-child').addClass('waitBtn');
									...
									_ajaxCallJsonSynch(
											"com/stpl/pms/action/bo/reports/st_pms_bo_rep_searchMISReportOnClick.action?reportData="
													+ $(
															"#reportData")
															.val().slice(0, -1)+',"proceName":"StCmsReferFriendVisitedTracking","startIndex":"'+$(
															'#startIndex')
															.text()+'","recordTofetch":"'+$(
															"#recordTofetch")
															.val()+'","campId":"'+$(
							...
							$('#reportTable_vis_previous').live('click', function(e) {
									if($("[data-dt-idx='1']").text()>1){
 										$("#reportTable_vis_previous").removeClass("disabled");
 									}
									...
									if($("[data-dt-idx='1']").hasClass("prevCall") && !($("#reportTable_vis_previous").hasClass("disabled"))) {	
									if(parseInt($("#startIndex").text())>0)
											...
											$("#nextPageStart").val(nextPageStart);
									$('#startIndex').text(start);
									 y = document.getElementById("reportTable_vis_paginate").firstChild.innerHTML;
									document.getElementById("reportTable_vis_paginate").firstChild.innerHTML=document.getElementById("waitDivPrev").innerHTML;
									$('#reportTable_vis_paginate > a:first-child').addClass('waitBtn');
									$(".paginate_button").css("pointer-events","none");
									_ajaxCallJsonSynch(
											"com/stpl/pms/action/bo/reports/st_pms_bo_rep_searchMISReportOnClick.action?reportData="
													+ $(
		...
		var resultList = response;
			...
			if ($("#rowCount").text() < 101){
				$("#reportTable_vis_next").addClass("disabled");}
			//	alert(response.size());
				var nextPage = $("#nextPageStart").val();
				
			jQuery('#reportTable_vis').dataTable().fnDestroy();
			$("#" + 'reportTable_vis')
					.DataTable(
							{
								"sAjaxSource" : 'com/stpl/pms/action/bo/reports/dataTablesActionMis.action',
								...
								"bSort" : false,
								 "fnInitComplete": function( oSettings) {
 									$("#reportTable_vis_previous").removeClass("disabled");
 									$("[data-dt-idx='1']").addClass('prevCall');
 									document.getElementById("reportTable_vis_paginate").lastChild.innerHTML = x;
									$($(".dataTables_paginate>span>.paginate_button")).each(function(){
									$(this).text(nextPage);
									nextPage++;
								});
					...
					.DataTable(
							{
								...
								"bSort" : false,
								 "fnInitComplete": function( oSettings) {
									 if($('#startIndex').val()==0){
										$("#reportTable_vis_previous").addClass("disabled");}
									 else
 										$("#reportTable_vis_previous").removeClass("disabled");
 									$("[data-dt-idx='1']").addClass('prevCall');
 									document.getElementById("reportTable_vis_paginate").firstChild.innerHTML = y;
									$($(".dataTables_paginate>span>.paginate_button")).each(function(){
									$(this).text(nextPage);
									nextPage++;
									$('#reportTable_vis').dataTable().fnPageChange( 'last' );
									$("#reportTable_vis_next").removeClass("disabled");
									$("[data-dt-idx='5']").addClass('nextCall');
								});
								},
			...
			<s:if test="%{resultList!=null && resultList.size()>0}">
				...
				<div class="innerBox">
					<table cellspacing="0" cellpadding="4" border="0"
						align="center" class="payTransaction" id="reportTable_vis">
						<thead>
						  <tr class="headerRow">
