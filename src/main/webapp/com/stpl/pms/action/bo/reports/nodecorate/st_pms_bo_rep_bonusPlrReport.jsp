			 ...
			 $("#subReportTable").DataTable({
				...
				"bVisible": true, 
 				...
 				$("#subReportTable_next").live('click', function(e) {
								...
								if($("[aria-controls='subReportTable'][data-dt-idx='5']").hasClass("nextCall")  && !($("#subReportTable_next").hasClass("disabled"))) {
									...
									_ajaxCallJsonSynch(
											"com/stpl/pms/action/bo/reports/st_pms_bo_rep_searchMISReportOnClick.action?reportData="
													+ $(
															"#reportData")
															.val().slice(0, -1)+',"proceName":"getBonusPlrDetailReport","startIndex":"'+$('#startIndexSub').text()+'","recordTofetch":"'+$('#recordTofetchSub').text()+'","bonusCode":"'+$('#bonusCodeSub').text()+'"}'
												,
											"",
											function(response) {
							...
							$(document).on("keyup", "[aria-controls='subReportTable']#search", function(){
								if($(this).val()!=""){
...
<body>
	...
	<div class="FormSection">
		<s:if test="%{resultList!=null && resultList.size()>0}">
			<div class="greyStrip">
				<h4 style="float: left">
					Bonus Usage Player Report For :
					<s:property value="%{bonusName}" />
				</h4>
			...
			<div class="innerBox">
				<table cellspacing="0" cellpadding="4" border="0" align="center"
					...
					<thead>
						<tr class="headerRow">
							<th style="text-align: center;" rowspan="1">Player Id</th>
			...
			<div id=recordTofetchSub style="display:none"><s:property value="101" /></div>
		 	...
		 	<div id="bonusCodeSub" style="display:none"><s:property value="%{bonusName}" /></div>
		</s:if>
		<s:else>
			<div class="greyStrip">
				<h4>Error Message</h4>
