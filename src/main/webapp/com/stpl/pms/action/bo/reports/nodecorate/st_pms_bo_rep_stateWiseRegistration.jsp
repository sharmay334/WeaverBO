...
<script>
		...
		$('#waitDiv').hide();
  		...
  		fromAndToDate();
  		$("#reportTableRegistartion").DataTable({
			...
			"oLanguage" : {
			 ...
			 columnDefs: [
			              { type: 'date-dd-mmm-yyyy hh:mm:ss', targets: 4 }
			            ]
			    ...
			    $(document).on("click", "[data-dt-idx='5']:not(.next,.previous)", function(){
 					if($("#rowCount").text()==101 && $("#search").val()==""){
	 					$("#reportTableRegistartion_next").removeClass("disabled");
		 				$("[data-dt-idx='5']").addClass('nextCall');
		 			}
 				});
 				$(document).on("click", "[data-dt-idx='1']:not(.next,.previous)", function(){
 					if($("[data-dt-idx='1']").text()>1)
 						$("#reportTableRegistartion_previous").removeClass("disabled");
	 				$("[data-dt-idx='1']").addClass('prevCall');
 				});
 				$("#reportTableRegistartion_next").live('click', function(e) {
								if($("#rowCount").text()==101 && $("#search").val()==""){
									$("#reportTableRegistartion_next").removeClass("disabled");
								}
								if($("#reportTableRegistartion_next").hasClass("disabled"))
									return false;
								...
								if($("[data-dt-idx='5']").hasClass("nextCall")  && !($("#reportTableRegistartion_next").hasClass("disabled"))) {
									var start = parseInt($(
											"#startIndex")
											.val()) + 100;
											flagPage = 0;
									...
									$('#startIndex').val(start);
									 x = document.getElementById("reportTableRegistartion_paginate").lastChild.innerHTML;
									document.getElementById("reportTableRegistartion_paginate").lastChild.innerHTML=document.getElementById("waitDivNext").innerHTML;
									$('#reportTableRegistartion_paginate > a:last-child').addClass('waitBtn');
									...
									_ajaxCallJsonSynch(
											"com/stpl/pms/action/bo/reports/st_pms_bo_rep_searchMISReportOnClick.action?reportData="
													+ $(
															"#reportData")
															.val().slice(0, -1)+',"proceName":"StateWiseReport"}'
													+ "&startIndex="
													+ $(
															'#startIndex')
															.val()
							...
							$('#reportTableRegistartion_previous').live('click', function(e) {
									if($("[data-dt-idx='1']").text()>1){
 										$("#reportTableRegistartion_previous").removeClass("disabled");
 									}
									...
									if($("[data-dt-idx='1']").hasClass("prevCall") && !($("#reportTableRegistartion_previous").hasClass("disabled"))) {	
									var start = parseInt($("#startIndex").val()) - 100;
											var nextPageStart = parseInt($('.paginate_button.current').text())-5;
											$("#nextPageStart").val(nextPageStart);
									$('#startIndex').val(start);
									 y = document.getElementById("reportTableRegistartion_paginate").firstChild.innerHTML;
									document.getElementById("reportTableRegistartion_paginate").firstChild.innerHTML=document.getElementById("waitDivPrev").innerHTML;
									$('#reportTableRegistartion_paginate > a:first-child').addClass('waitBtn');
									$(".paginate_button").css("pointer-events","none");
									_ajaxCallJsonSynch(
											"com/stpl/pms/action/bo/reports/st_pms_bo_rep_searchMISReportOnClick.action?reportData="
													+$(
		...
		var resultList = response;
			...
			if ($("#rowCount").text() < 101){
				$("#reportTableRegistartion_next").addClass("disabled");}
			//	alert(response.size());
				var nextPage = $("#nextPageStart").val();
				
			jQuery('#reportTableRegistartion').dataTable().fnDestroy();
			$("#" + 'reportTableRegistartion')
					.DataTable(
							{
								"sAjaxSource" : 'com/stpl/pms/action/bo/reports/dataTablesActionMis.action',
								"sAjaxDataProp" : "resultList",
								...
								//"bSort" : false,
								 "fnInitComplete": function( oSettings) {
 									$("#reportTableRegistartion_previous").removeClass("disabled");
 									$("[data-dt-idx='1']").addClass('prevCall');
 									document.getElementById("reportTableRegistartion_paginate").lastChild.innerHTML = x;
									$($(".dataTables_paginate>span>.paginate_button")).each(function(){
									$(this).text(nextPage);
									nextPage++;
								});
					...
					.DataTable(
							{
								...
								//"bSort" : false,
								 "fnInitComplete": function( oSettings) {
									 if($('#startIndex').val()==0){
										$("#reportTableRegistartion_previous").addClass("disabled");}
									 else
 										$("#reportTableRegistartion_previous").removeClass("disabled");
 									$("[data-dt-idx='1']").addClass('prevCall');
 									document.getElementById("reportTableRegistartion_paginate").firstChild.innerHTML = y;
									$($(".dataTables_paginate>span>.paginate_button")).each(function(){
									$(this).text(nextPage);
									nextPage++;
									$('#reportTableRegistartion').dataTable().fnPageChange( 'last' );
									$("#reportTableRegistartion_next").removeClass("disabled");
									$("[data-dt-idx='5']").addClass('nextCall');
								});
								},
		...
		<s:if test="%{resultList!=null && resultList.size()>0}">
			...
			<div class="innerBox">
				<table cellspacing="0" cellpadding="4" border="0" align="center"
					class="payTransaction" id="reportTableRegistartion">
					<thead>
