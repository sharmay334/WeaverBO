...
$(document).ready(function(){
		...
		if(isData){			
			 $chk = $('<th valign="middle" style="text-align: center;" nowrap="nowrap"> Player Id</th>	'  );
			 $('#divId').append($chk);
  		...
  		fromAndToDate();
  		$("#reportTableAllPlayer").DataTable({
			...
			"oLanguage" : {
			    ...
			    $(document).on("click", "[data-dt-idx='5']:not(.next,.previous)", function(){
 					if($("#rowCount").text()==101 && $("#search").val()==""){
	 					$("#reportTableAllPlayer_next").removeClass("disabled");
		 				$("[data-dt-idx='5']").addClass('nextCall');
		 			}
 				});
 				$(document).on("click", "[data-dt-idx='1']:not(.next,.previous)", function(){
 					if($("[data-dt-idx='1']").text()>1)
 						$("#reportTableAllPlayer_previous").removeClass("disabled");
	 				$("[data-dt-idx='1']").addClass('prevCall');
 				});
 				$("#reportTableAllPlayer_next").live('click', function(e) {
								if($("#rowCount").text()==101 && $("#search").val()==""){
									$("#reportTableAllPlayer_next").removeClass("disabled");
								}
								if($("#reportTableAllPlayer_next").hasClass("disabled"))
									return false;
								...
								if($("[data-dt-idx='5']").hasClass("nextCall")  && !($("#reportTableAllPlayer_next").hasClass("disabled"))) {
									var start = parseInt($(
											"#startIndex")
											.val()) + 100;
											flagPage = 0;
									...
									$('#startIndex').val(start);
									 x = document.getElementById("reportTableAllPlayer_paginate").lastChild.innerHTML;
									document.getElementById("reportTableAllPlayer_paginate").lastChild.innerHTML=document.getElementById("waitDivNext").innerHTML;
									$('#reportTableAllPlayer_paginate > a:last-child').addClass('waitBtn');
							...
							$('#reportTableAllPlayer_previous').live('click', function(e) {
									if($("[data-dt-idx='1']").text()>1){
 										$("#reportTableAllPlayer_previous").removeClass("disabled");
 									}
									...
									if($("[data-dt-idx='1']").hasClass("prevCall") && !($("#reportTableAllPlayer_previous").hasClass("disabled"))) {	
									var start = parseInt($("#startIndex").val()) - 100;
											var nextPageStart = parseInt($('.paginate_button.current').text())-5;
											$("#nextPageStart").val(nextPageStart);
									$('#startIndex').val(start);
									 y = document.getElementById("reportTableAllPlayer_paginate").firstChild.innerHTML;
									document.getElementById("reportTableAllPlayer_paginate").firstChild.innerHTML=document.getElementById("waitDivPrev").innerHTML;
									$('#reportTableAllPlayer_paginate > a:first-child').addClass('waitBtn');
									$(".paginate_button").css("pointer-events","none");
									_ajaxCallJsonSynch(
											"com/stpl/pms/action/bo/reports/st_pms_bo_rep_searchMISReportOnClick.action?reportData="
													+$(
		...
		var resultList = response;
			...
			if ($("#rowCount").text() < 101){
				$("#reportTableAllPlayer_next").addClass("disabled");}
			//	alert(response.size());
				var nextPage = $("#nextPageStart").val();
				
			jQuery('#reportTableAllPlayer').dataTable().fnDestroy();
			$("#" + 'reportTableAllPlayer')
					.DataTable(
							{
								"sAjaxSource" : 'com/stpl/pms/action/bo/reports/dataTablesActionMis.action',
								"sAjaxDataProp" : "resultList",
								...
								"bSort" : false,
								 "fnInitComplete": function( oSettings) {
 									$("#reportTableAllPlayer_previous").removeClass("disabled");
 									$("[data-dt-idx='1']").addClass('prevCall');
 									document.getElementById("reportTableAllPlayer_paginate").lastChild.innerHTML = x;
									$($(".dataTables_paginate>span>.paginate_button")).each(function(){
									$(this).text(nextPage);
									nextPage++;
								});
					...
					.DataTable(
							{
								...
								"bSort" : false,
								 "fnInitComplete": function( oSettings) {
									 if($('#startIndex').val()==0){
										$("#reportTableAllPlayer_previous").addClass("disabled");}
									 else
 										$("#reportTableAllPlayer_previous").removeClass("disabled");
 									$("[data-dt-idx='1']").addClass('prevCall');
 									document.getElementById("reportTableAllPlayer_paginate").firstChild.innerHTML = y;
									$($(".dataTables_paginate>span>.paginate_button")).each(function(){
									$(this).text(nextPage);
									nextPage++;
									$('#reportTableAllPlayer').dataTable().fnPageChange( 'last' );
									$("#reportTableAllPlayer_next").removeClass("disabled");
									$("[data-dt-idx='5']").addClass('nextCall');
								});
								},
		...
		<s:if test="%{resultList!=null && resultList.size()>0}">
			...
			<div class="innerBox">
				<table cellspacing="0" cellpadding="4" border="0" align="center"
					class="payTransaction" id="reportTableAllPlayer">
					<thead>
