			 ...
			 $("#subReportTable").DataTable({
				...
				"bVisible": true, 
 				...
 				$(document).on("click", "[aria-controls='subReportTable'][data-dt-idx='5']:not(.next,.previous)", function(){
 					if($("#rowCountSub").text()==101 && $("[aria-controls='subReportTable'][id='search']").val()==""){
 				...
 				$("#subReportTable_next").live('click', function(e) {
								...
								if($("[aria-controls='subReportTable'][data-dt-idx='5']").hasClass("nextCall")  && !($("#subReportTable_next").hasClass("disabled"))) {
									var start = parseInt($(
											...
											.text()) + 100;
											flagPage = 0;
											var nextPageStart = parseInt($("[aria-controls='subReportTable'][data-dt-idx='5']").text())+1;
									...
									$("#subReportTable_paginate").css("pointer-events","none");
									var aliasVal= $("#domainId").val()==0?-1: $("#aliasId").val();
									_ajaxCallJsonSynch(
											'com/stpl/pms/action/bo/reports/st_pms_bo_rep_searchMISReportOnClick.action?reportData={"proceName":"CampWiseRevenuReport","domainId":'+$("#domainId").val()+',"aliasId":'+aliasVal+',"camType":"'+$("#camType").val()+'","camName":"'+$("#camName").text()+'","'+'subCamName":"'+$("#subCamName").text()+'","startIndex":"'+$("#startIndexSub").text()+'","'+'recordTofetch":"'+$("#recordTofetchSub").text()+'"}'
												,
											"",
											function(response) {
							...
							$(document).on("keyup", "[aria-controls='subReportTable'][id='search']", function(){
								if($(this).val()!=""){
								...
								$("#subReportTable_paginate").css("pointer-events","auto");
					...
					.DataTable(
							{
								...
								"fnDrawCallback" : function(oSettings) {
									if($("[aria-controls='subReportTable'][id='search']").val()==""){
								flagPage++;
								if(flagPage < 3) {
									return false;
								}
		 	...
		 	<div id="nextPageStartSub" style="display:none"></div> 
				...
				<%-- <div id="startIndex" style="display:none"><s:property value="0" /></div>
			 	<div id="rowCountSub" style="display:none"><s:property value="%{resultList.size()}"/></div> 
			  	...
			  	<div id="camName" style="display:none"><s:property value="%{campName}" /></div>
			  	<div id="subCamName" style="display:none"><s:property value="%{subCampName}" /></div>
